...
def heat_control_hysteresis_thresh(temp_measured, current_state, temp_desired, alpha):
    ...
    #temp_measured<temp_desired
    #temp_measured>temp_desired    
    
    
    
    #math.abs(temp_measured-temp_desired)>=alpha
...
def thresh_crossing_counter(temp_desired, hyst_alpha,
    ...
    counter=0
    t=t_start
    new_state=get_sensor_measurement(t+t_step,c0,c1,c2,c3,c4)
    if new_state>(temp_desired+hyst_alpha):
            new_state_bool=True  
    elif new_state<=(temp_desired-hyst_alpha):
            new_state_bool=False  
    if not new_state_bool==state :
            counter= counter + 1    
    t=t+t_step
    while t<=(t_stop-0.05) :
        state=get_sensor_measurement(t,c0,c1,c2,c3,c4)
        new_state=get_sensor_measurement(t+t_step,c0,c1,c2,c3,c4)    
        if state>(temp_desired+hyst_alpha):
            state_bool=True

        elif state<=(temp_desired-hyst_alpha):
            state_bool=False         
        ...
        if new_state>(temp_desired+hyst_alpha):
            ...
            new_state_bool=False             
        if not new_state_bool==state_bool :
                counter+=1        
        t=t+t_step  
    return counter


print( thresh_crossing_counter(4,1,-4,0,1,2,3,4,5))
